...
for sev_folder in [os.path.join(sample_folder, sev) for sev in severities]:
    for project_folder in [f.path for f in os.scandir(sev_folder) if f.is_dir()]:
        ...
        if not os.path.exists(os.path.join(analysis_folder, 'diff.csv')):
            continue
        if not os.path.exists(os.path.join(analysis_folder, 'log-distances.csv')):
            temp_df = pd.read_csv(os.path.join(analysis_folder, 'distances_realloc.csv'))
            ...
            temp_df.to_csv(os.path.join(analysis_folder, 'alldist.csv'))
            continue
        temp_df = pd.read_csv(os.path.join(analysis_folder, 'distances_realloc.csv'))
        log_df=pd.read_csv(os.path.join(analysis_folder, 'log-distances.csv'))
